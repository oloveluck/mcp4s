...
/** Simple mock LLM that pattern-matches on prompts. In a real application, this would call an
  * actual LLM API.
  */
object MockLLM:
  ...
  private def extractAndCalculate(
      prompt: String,
...
object ElicitationHandlers:
  ...
  def elicitationCompleteHandler: ElicitationCompleteParams => IO[Unit] =
    params =>
      IO.println(
        s"[ElicitationComplete] ID: ${params.elicitationId}, Action: ${params.result.action}"
      ) *>
...
object DemoServer extends IOApp.Simple:
  ...
  val tools: Tools[IO] =
    ...
    Tool.from[DemoAddArgs].withName("add").handle[IO] { args =>
...
/** Demo client with sampling and elicitation handlers, built with the composable DSL. */
object DemoClient extends IOApp.Simple:

  def createClient: McpClient[IO] =
    McpClient.from[IO](
      ClientInfo("sampling-demo-client", "1.0.0"),
      sampling = Some(Samplings[IO](SamplingHandlers.mockLLMHandler)),
      elicitation = Some(
        Elicitations.withComplete[IO](
          ElicitationHandlers.simulatedHandler,
          ElicitationHandlers.elicitationCompleteHandler
        )
      )
    )

  def run: IO[Unit] =
    IO.println("Demo Client (DSL) created with:") *>
