...
object ConformanceServer extends IOApp.Simple:
  ...
  val simpleTools: Tools[IO] =
    mcp.Tool.text[IO]("test_simple_text", "Tests simple text content response") {
      ...
      mcp.Tool
        .withContext[IO]("test_embedded_resource", "Tests embedded resource content response") {
          _ =>
            ToolResult(
              List(
                ResourceContentRef(
      ...
      mcp.Tool.withContext[IO](
        "test_multiple_content_types",
        "Tests response with multiple content types"
      ) { _ =>
        ToolResult(
          List(
      ...
      mcp.Tool
        .text[IO]("test_reconnection", "Tests SSE stream disconnection and client reconnection") {
  ...
  val contextTools: Tools[IO] =
    mcp.Tool.withContext[IO](
      ...
      mcp.Tool.withContext[IO, SamplingArgs]("test_sampling", "Tests server-initiated sampling") {
        (args, ctx) =>
      ...
      mcp.Tool.withContext[IO, ElicitationArgs](
        "test_elicitation",
        "Tests server-initiated elicitation"
      ) { (args, ctx) =>
        ctx.elicitation
          .elicit(
            ElicitFormParams(
      ...
      mcp.Tool.withContext[IO](
        ...
        "Tests elicitation with enum schema per SEP-1330"
      ) { ctx =>
        ctx.elicitation
          .elicit(
            ElicitFormParams(
  ...
  val argPrompts: Prompts[IO] =
    mcp.Prompt[IO, PromptWithArgsInput](
    ...
    |+|
      mcp.Prompt[IO, EmbeddedResourcePromptInput](
