...
class WebSocketIntegrationSpec extends CatsEffectSuite:
  ...
  def createTestServer: Server[IO] =
    val tools =
      Tools.single[IO](testTool) { args =>
        ...
        Tools.singleWithContext[IO](
          Tool("slow_multiply", Some("Multiply with progress"), JsonSchema.empty)
        ) { (args, ctx) =>
          val cursor = args.hcursor
          for
            a <- cursor.get[Double]("a").liftTo[IO]
    ...
    val resources = Resources.single[IO](testResource)(_ =>
      IO.pure(ResourceContent.text("file:///ws-test.txt", "WebSocket test content"))
    )
    ...
    Server.from[IO](ServerInfo("ws-test-server", "1.0.0"), tools, resources, prompts)

  def testClient: McpClient[IO] = McpClient.from[IO](ClientInfo("ws-test-client", "1.0.0"))
  ...
  test("WebSocket: client reads resource from server") {
    wsServerResource.use: server =>
      val port = server.address.getPort
      wsConnectedClient(port).use: conn =>
        for content <- conn.readResource("file:///ws-test.txt")
        yield
  ...
  test("WebSocket: client gets prompt from server") {
    wsServerResource.use: server =>
      val port = server.address.getPort
      wsConnectedClient(port).use: conn =>
        for result <- conn.getPrompt("ws-greeting", Map("name" -> "WebSocket"))
        yield
