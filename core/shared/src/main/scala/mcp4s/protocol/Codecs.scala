...
/** Circe codecs for JSON-RPC and MCP protocol types Spec ref:
  * https://modelcontextprotocol.io/specification/2025-11-25/schema
  */
object Codecs:

  /** The standard derived object encoder used throughout: absent (null) optional fields are omitted
    * from the output, per the MCP wire convention.
    */
  private inline def derivedEncoder[A](using inline m: Mirror.Of[A]): Encoder[A] =
    deriveEncoder[A].mapJson(_.dropNullValues)
  ...
  given Encoder[JsonRpcError] = derivedEncoder[JsonRpcError]
  ...
  given Encoder[Icon] = derivedEncoder[Icon]
  given Decoder[Icon] = deriveDecoder

  given Encoder[Annotations] = derivedEncoder[Annotations]
  ...
  given Encoder[ToolsCapability] = derivedEncoder[ToolsCapability]
  given Decoder[ToolsCapability] = deriveDecoder

  given Encoder[ResourcesCapability] = derivedEncoder[ResourcesCapability]
  given Decoder[ResourcesCapability] = deriveDecoder

  given Encoder[PromptsCapability] = derivedEncoder[PromptsCapability]
  ...
  // Server tasks
  given Encoder[ToolTaskRequests] = derivedEncoder[ToolTaskRequests]
  given Decoder[ToolTaskRequests] = deriveDecoder

  given Encoder[ServerTaskRequests] = derivedEncoder[ServerTaskRequests]
  given Decoder[ServerTaskRequests] = deriveDecoder

  given Encoder[ServerTasksCapability] = derivedEncoder[ServerTasksCapability]
  given Decoder[ServerTasksCapability] = deriveDecoder

  given Encoder[ServerCapabilities] = derivedEncoder[ServerCapabilities]
  given Decoder[ServerCapabilities] = deriveDecoder

  // Client tasks
  given Encoder[SamplingTaskRequests] = derivedEncoder[SamplingTaskRequests]
  given Decoder[SamplingTaskRequests] = deriveDecoder

  given Encoder[ElicitationTaskRequests] = derivedEncoder[ElicitationTaskRequests]
  given Decoder[ElicitationTaskRequests] = deriveDecoder

  given Encoder[ClientTaskRequests] = derivedEncoder[ClientTaskRequests]
  given Decoder[ClientTaskRequests] = deriveDecoder

  given Encoder[ClientTasksCapability] = derivedEncoder[ClientTasksCapability]
  given Decoder[ClientTasksCapability] = deriveDecoder

  given Encoder[ElicitationCapability] = derivedEncoder[ElicitationCapability]
  given Decoder[ElicitationCapability] = deriveDecoder

  given Encoder[RootsCapability] = derivedEncoder[RootsCapability]
  given Decoder[RootsCapability] = deriveDecoder

  given Encoder[SamplingCapability] = derivedEncoder[SamplingCapability]
  given Decoder[SamplingCapability] = deriveDecoder

  given Encoder[ClientCapabilities] = derivedEncoder[ClientCapabilities]
  given Decoder[ClientCapabilities] = deriveDecoder

  // === JSON Schema ===

  given Encoder[JsonSchemaProperty] = derivedEncoder[JsonSchemaProperty]
  given Decoder[JsonSchemaProperty] = deriveDecoder

  given Encoder[JsonSchema] = derivedEncoder[JsonSchema]
  given Decoder[JsonSchema] = deriveDecoder

  // === Tools ===

  given Encoder[ToolAnnotations] = derivedEncoder[ToolAnnotations]
  ...
  given Encoder[ToolExecution] = derivedEncoder[ToolExecution]
  given Decoder[ToolExecution] = deriveDecoder

  given Encoder[Tool] = derivedEncoder[Tool]
  ...
  given Encoder[ToolResult] = derivedEncoder[ToolResult]
  given Decoder[ToolResult] = deriveDecoder

  // === Resources ===

  given Encoder[Resource] = derivedEncoder[Resource]
  given Decoder[Resource] = deriveDecoder

  given Encoder[ResourceTemplate] = derivedEncoder[ResourceTemplate]
  given Decoder[ResourceTemplate] = deriveDecoder

  given Encoder[ResourceContent] = derivedEncoder[ResourceContent]
  given Decoder[ResourceContent] = deriveDecoder

  // === Prompts ===

  given Encoder[PromptArgument] = derivedEncoder[PromptArgument]
  given Decoder[PromptArgument] = deriveDecoder

  given Encoder[Prompt] = derivedEncoder[Prompt]
  ...
  given Encoder[PromptMessage] = derivedEncoder[PromptMessage]
  given Decoder[PromptMessage] = deriveDecoder

  given Encoder[GetPromptResult] = derivedEncoder[GetPromptResult]
  given Decoder[GetPromptResult] = deriveDecoder

  // === Lifecycle ===

  given Encoder[InitializeParams] = derivedEncoder[InitializeParams]
  given Decoder[InitializeParams] = deriveDecoder

  given Encoder[InitializeResult] = derivedEncoder[InitializeResult]
  ...
  given Encoder[LogMessage] = derivedEncoder[LogMessage]
  ...
  given Encoder[CancelledParams] = derivedEncoder[CancelledParams]
  given Decoder[CancelledParams] = deriveDecoder

  given Encoder[ProgressParams] = derivedEncoder[ProgressParams]
  given Decoder[ProgressParams] = deriveDecoder

  // === Roots (Client Feature) ===

  given Encoder[Root] = derivedEncoder[Root]
  given Decoder[Root] = deriveDecoder

  given Encoder[ListRootsResult] = derivedEncoder[ListRootsResult]
  ...
  given Encoder[ModelHint] = derivedEncoder[ModelHint]
  given Decoder[ModelHint] = deriveDecoder

  given Encoder[ModelPreferences] = derivedEncoder[ModelPreferences]
  ...
  given Encoder[CreateMessageParams] = derivedEncoder[CreateMessageParams]
