...
/** Typeclass for tool input types that can be decoded from JSON and have a schema.
  *
  * Provides automatic derivation for case classes using `derives ToolInput`. Supports
  * `@description` annotations on fields for JSON schema documentation.
  *
...
object ToolInput:
  ...
  /** Derive ToolInput for a product type (case class).
    *
    * This automatically derives the Decoder using Circe's generic derivation, so you don't need a
    * separate `given Decoder[A]`.
    */
