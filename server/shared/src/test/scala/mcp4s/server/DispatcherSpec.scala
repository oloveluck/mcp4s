...
class DispatcherSpec extends CatsEffectSuite:
  ...
  def testServer: Server[IO] = new Server[IO]:
    ...
    def readResource(uri: String): IO[ResourceContent] =
      if uri == "file:///test.txt" then
        IO.pure(ResourceContent.text("file:///test.txt", "Test content"))
        ...
        IO.pure(
          GetPromptResult(
            ...
            List(
              PromptMessage(
                Role.User,
                TextContent(s"Hello, ${arguments.getOrElse("name", "World")}!")
              )
            )
          )
        )
  ...
  def progressServer: Server[IO] = new Server[IO]:
    ...
    def listTools: IO[List[Tool]] = IO.pure(
      List(
