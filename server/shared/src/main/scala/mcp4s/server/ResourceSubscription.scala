...
/** Manages resource subscriptions for MCP servers.
  *
  * Tracks which clients (identified by session ID) are subscribed to which resources, and provides
  * methods to notify subscribers when resources change.
  *
  * Example usage:
  * {{{
  * for
...
trait ResourceSubscriptionManager[F[_]]:
  ...
  /** Get all sessions subscribed to a resource.
    ...
    *   The resource URI
    * @return
    *   Set of session IDs subscribed to the resource
    */
  ...
  /** Get all resources a session is subscribed to.
    ...
    *   The session ID
    * @return
    *   Set of resource URIs the session is subscribed to
    */
  def getSubscriptions(sessionId: String): F[Set[String]]

  /** Stream of resource update notifications.
    *
    * Returns a stream that emits (sessionId, uri) pairs when resources change. Consumers can use
    * this to send notifications to the appropriate sessions.
    */
  def notifications: Stream[F, (String, String)]

  /** Connect to a Resources change stream.
    *
    * Returns a stream that monitors resource changes and notifies subscribers. This stream should
    * be run concurrently with the server.
    *
    * Example:
    * {{{
...
object ResourceSubscriptionManager:
  ...
  /** Create a new subscription manager.
    *
    * @param maxQueueSize
    *   Maximum number of pending notifications (bounded with backpressure)
    */
  ...
  private class ResourceSubscriptionManagerImpl[F[_]: Concurrent](
    ...
    def notifyChanged(uri: String): F[Unit] =
      getSubscribers(uri).flatMap: subscribers =>
        subscribers.toList.traverse_(sessionId => notificationQueue.offer((sessionId, uri)))
