...
object HttpClientTransport:
  ...
  def connect[F[_]: Async](
    ...
    for connection <- CatsResource.eval(
        establishConnection(client, httpClient, config, summon[Tracer[F]])
      )
    yield connection
  ...
  private def establishConnection[F[_]: Async](
    ...
    for
      ...
      progressHandlersRef <- Ref
        .of[F, Option[Ref[F, Map[RequestId, ProgressParams => F[Unit]]]]](None)

      // Create the request sender function with session and trace context
      sendRequest = createRequestSender(
        httpClient,
        endpointUri,
        sessionIdRef,
        progressHandlersRef,
      ...
      sendNotification = createNotificationSender(
        httpClient,
        endpointUri,
        sessionIdRef,
        config,
  ...
  private def runRequestToJson[F[_]: Async](
    ...
    httpClient
      ...
      .use: response =>
        ...
        val parsedJson =
          if !sse then response.asJson
          else
            response.body
              .through(org.http4s.ServerSentEvent.decoder[F])
              .evalMap { case ServerSentEvent(data, eventType, id, retry, comment) =>
                data
                  .map(utf8String =>
                    io.circe.parser.parse(utf8String) match
                      ...
                      case Right(json) =>
                        ...
                        json.as[JsonRpcMessage] match
                          case Right(notif: JsonRpcNotification)
                              if notif.method == McpMethod.Progress =>
                            val routed = for
