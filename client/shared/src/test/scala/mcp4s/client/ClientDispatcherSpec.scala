...
class ClientDispatcherSpec extends CatsEffectSuite:
  ...
  def testClient: McpClient[IO] =
    McpClient.from[IO](
      ClientInfo("test-client", "1.0.0"),
      roots = Some(Roots[IO](testRoots*)),
      sampling = Some(Samplings[IO](_ => IO.pure(testSamplingResult))),
      elicitation = Some(Elicitations[IO](_ => IO.pure(testElicitResult)))
    )

  def minimalClient: McpClient[IO] = McpClient.from[IO](ClientInfo("mcp4s-client", "0.1.0"))
  ...
  def sendRequest(
      dispatcher: ClientDispatcher[IO],
      method: String,
