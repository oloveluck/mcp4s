...
class McpConnectionSpec extends CatsEffectSuite:
  ...
  def mockResponse(method: String, response: Json): JsonRpcRequest => IO[Json] =
    req =>
      if req.method == method then IO.pure(response)
      else IO.raiseError(new RuntimeException(s"Unexpected method: ${req.method}"))
  ...
  test("request IDs are incremented") {
    ...
    for
      ...
      _ <- conn.listTools()
      _ <- conn.listTools()
    yield assertEquals(ids.toList, List(1L, 2L, 3L))
  }
  ...
  test("progress handler is registered and cleaned up after request completes") {
    ...
    yield assert(handlers.isEmpty, "Progress handlers should be cleaned up after request completes")
  }
  ...
  test("progress handler receives notifications routed by transport") {
    ...
    for
      ...
      _ <- conn.progressHandlers.update(
        _ + (RequestId.NumberId(42) ->
