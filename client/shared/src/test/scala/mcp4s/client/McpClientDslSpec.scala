...
class McpClientDslSpec extends CatsEffectSuite:
  ...
  test("Sampling creates composable handler") {
    ...
    for result <- sampling.handle(params).value
    yield
  ...
  test("Elicitation creates composable handler") {
    ...
    for result <- elicitation.handle(params).value
    yield
      assert(result.isDefined)
  ...
  test("Elicitation.withComplete includes complete handler") {
    ...
    val completeParams =
      ElicitationCompleteParams("test-id", ElicitResult(ElicitAction.Accept, None))

    for _ <- elicitation.onComplete(completeParams)
  ...
  test("McpClient.from with sampling handles requests") {
    ...
    for result <- client.createMessage(params)
    yield assertEquals(result.model, "test-model")
  }
  ...
  test("McpClient.from with elicitation handles requests") {
    ...
    for result <- client.elicit(params)
  ...
  test("McpClient.from composes multiple roots") {
    val client = McpClient.from[IO](
      ClientInfo("test", "1.0.0"),
      roots = Some(Roots[IO](Root("file:///a", Some("A")), Root("file:///b", Some("B"))))
    )

    assert(client.capabilities.roots.isDefined)
    for result <- client.listRoots
    yield assertEquals(result.roots.map(_.uri), List("file:///a", "file:///b"))
  }
